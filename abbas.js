...
cincopa.json_stringify = JSON.stringify || (function () { // some sites override JSON.stringify for some reason
	var iframe = document.createElement('iframe');
  	document.body.appendChild(iframe);
  	var retrieved = iframe.contentWindow["JSON"];
  	document.body.removeChild(iframe);
  	return retrieved.stringify;
})()
cincopa.json_parse = JSON.parse || (function () {
  	var iframe = document.createElement('iframe');
  	document.body.appendChild(iframe);
  	var retrieved = iframe.contentWindow["JSON"];
  	document.body.removeChild(iframe);
  	return retrieved.parse;
})()
...
cincopa.is_ie = function(){
	return navigator.userAgent.match(/MSIE/) || !!navigator.userAgent.match(/Trident.*rv\:11\./)
}
cincopa.hasTouch = function(){
    var ua = navigator.userAgent.toLowerCase();
	return ('ontouchstart' in window && ua.match(/chrome/gi) === null) || ("ontouchstart" in window && ua.match(/chrome/gi) !== null && /android|iphone|ipad|ipod/i.test(ua));
}
...
cincopa._JSONV2 = cincopa._JSONV2 || (cincopa._ROOT_PROTOCOL + "//rtcdn.cincopa.com/jsonv2.aspx");
...
cincopa.logError = function (what, errname) {
	try {
		...
		oat.src = cincopa._ROOT_ANALYTICS + "/oa.aspx?uid=rtlogerror&fid=" + what + "&setref=" + encodeURIComponent("http://" + errname + "/" + cincopa.location.replace("?", "@").replace("#", "@"));
		cincopa.traceit("CINCOPA LOG ERROR : " + what + " data: " + errname, "");
	} catch (ex) { }
}
...
cincopa.cprtv2 = function (go) {
	...
	var template = null;
	var url = cincopa._ROOT_PROTOCOL + "//rtcdn.cincopa.com/meta_json.aspx";
	...
	if (cincopa.qs["cprtv3"]) {
		Ajax(url, { http_method: "GET" }, function (res) {
			var metajson = cincopa.json_parse(res);
			go.acc = metajson.acc;
		...
		xhttp.onreadystatechange = function () {
			if (this.readyState == 4 && this.status == 200) {
				var metajson = cincopa.json_parse(this.responseText);
				go.acc = metajson.acc;

				if (metajson.args.old_skin == "true") {
					cincopa.traceit("Cincopa : Old skin detected");
					var setref = "";
					if (cincopa.location_blocked)
						setref = "&setref=" + encodeURIComponent(cincopa.location);
...
cincopa.boot_gallery = function (zeo) {
	...
	if (!getElement(zeo["_object"])) {
		...
		cincopa.logError("trackmissingdiv1", zeo["_fid"]);
...
cincopa.testtrack = function (uid) {
	...
	var lf = eval(atob(""));
...
function GalleryObject(params) {
	...
	this.arg_misc = {
		...
		allow_fixpos: { group: "misc", type: "list", name: "Fix Position", values: { no: "No (default)", cp_fixedLeftTop: "Left top", cp_fixedLeftBottom: "Left bottom", cp_fixedRightTop: "Right top", cp_fixedRightBottom: "Right bottom" }, desc: "Fix the gallery to the corner of the page.", enableExp: "param.allow_lightbox == 'no'" },
		allow_margins: { group: "misc", type: "num", name: "Gallery margins", desc: "Define the margin size of gallery from the screen.", enableExp: "param.allow_lightbox == 'no'" },
		allow_lightbox: { group: "misc", type: "list", name: "Open gallery in lightbox", values: { no: "No (default)", thumbnail: "thumbnail", text: "text" }, desc: "Open gallery in lightbox" },
		allow_lightbox_text: { group: "misc", type: "text", name: "Lightbox button text", desc: "Lightbox button text", enableExp: "param.allow_lightbox == 'text'" },
		allow_lightbox_size: { group: "misc", type: "list", name: "Lightbox thumbnail size", values: { small: "small 224:140", medium: "medium 440:275" }, desc: "Lightbox thumbnail size", enableExp: "param.allow_lightbox == 'thumbnail'" }
	};
	...
	this.arg_overlay_group = {
        overlay: { name: "Overlay", desc: "Overlay settings for the gallery." }
    };

    this.arg_overlay = {
        overlay_type: { group: "overlay", type: "list", name: "Overlay type", values: { off: "Off", button: "CTA button", watermark: "Watermark", poster_logo: "Logo on poster", custom: "Custom" }, desc: "Overlay type" },
        overlay_button_title: { group: "overlay", type: "text", name: "Overlay title", desc: "Overlay title", removeExp: "param.overlay_type != 'button'" },
        overlay_button_text: { group: "overlay", type: "text", name: "Button text", desc: "Text of button", removeExp: "param.overlay_type != 'button'" },
        overlay_button_color: { group: "overlay", type: "color", name: "Button color", desc: "Color of the button", removeExp: "param.overlay_type != 'button'" },
        overlay_button_text_color: { group: "overlay", type: "color", name: "Text color", desc: "Color of the text", removeExp: "param.overlay_type != 'button'" },
        overlay_watermark_url: { group: "overlay", type: "text", name: "Watermark image URL", desc: "Location of an external jpg, png or gif image to show in a corner of the display.", removeExp: "param.overlay_type != 'watermark'" },
        overlay_watermark_position: { group: "overlay", type: "list", name: "Watermark position", values: { top_left : "top left", top_right : "top right", bottom_left : "bottom left", bottom_right : "bottom right",}, desc: "Watermark Position",  removeExp: "param.overlay_type != 'watermark'" },
        overlay_link: { group: "overlay", type: "text", name: "Click url", desc: "Open URL on click.", removeExp: "param.overlay_type != 'button' && param.overlay_type != 'watermark'" },
        overlay_target: { group: "overlay", type: "bool", name: "Open url in new tab", desc: "Open link in current tab or new tab.", removeExp: "param.overlay_type != 'button' && param.overlay_type != 'watermark'" },
        overlay_html: { group: "overlay", type: "textarea", name: "Overlay html", desc: "Custom html of overlay", removeExp: "param.overlay_type != 'custom'" },
        overlay_poster_logo_url: { group: "overlay", type: "text", name: "Image URL", desc: "Location of an external jpg, png or gif image to show on poster", removeExp: "param.overlay_type != 'poster_logo'" },
        overlay_time: { group: "overlay", type: "list", name: "Overlay time", values: { onload: "on load", at_sec: "after X seconds", onhover: " on mouse hover", onclick: "on click" }, desc: "Overlay show time", removeExp: "param.overlay_type == 'off' || param.overlay_type == 'poster_logo'" },
        overlay_sec: { group: "overlay", type: "text", name: "Show delay", desc: "Delay in seconds for overlay appearance", removeExp: "param.overlay_time != 'at_sec'" },
    };

    this.arg_overlay_defaults = {
        overlay_type :"off",
	...
	this.onArgs = function (args) //
	{
		...
		if (this.acc) {
			if (this.acc.user) {
				...
				if (this.acc.user.status == "suspended") {
					cincopa.logError("user_suspended", this.uid);
				...
				else if (this.acc.user.status == "abuser") {
					cincopa.logError("user_abuser", this.uid);
					this.drawErrorBox("Your account has been flagged<br>Please <a href='https://www.cincopa.com/pricing'>upgrade</a> to continue using or contact us at sales@cincopa.com");
					return;
				}
				else if (this.acc.user.status == "outoftraffic") {
					cincopa.logError("user_outoftraffic", this.uid);
					this.drawErrorBox("Gallery not available<br>Please follow <a href='https://www.cincopa.com/pricing'>this link</a> or contact us at sales@cincopa.com");
					return;
				}
				else if (this.acc.user.status == "outofstorage" && this.acc.user.plan_name == "free") {
					cincopa.logError("user_free_outoftraffic", this.uid);
				...
				if (this.acc.user.integration && this.acc.user.integration.hubspot && this.acc.user.integration.hubspot == "active") {
					cincopa.registerEvent("cincopa.cp_hubspot_integration", "video.*");
					cincopa.registerEvent("cincopa.cp_hubspot_integration", "emailgate.*");
				}
			}
			else {
				cincopa.logError("gallery_not_ready", "unknow_user");
		...
		if (Object.keys && Object.keys(this.args).length === 0) {
			cincopa.logError("gallery_not_found", "unknow_user");
	...
	this.loadArgs = function () {
		...
		// base overlay args - copy all the defaults in case template_args changed them
		for (var n in this.arg_overlay_defaults)
			temp_args[n] = this.arg_overlay_defaults[n];
	...
	this.loadIFrameSkin = function () {
		...
		var ifrm = '<iframe id="zeiframe_' + this.loaderParams["_object"] + '" allowfullscreen scrolling=no frameborder=0 vspace=0 hspace=0 marginwidth=0 marginheight=0 width=' + w + 'px height=' + h + 'px></iframe>';
		...
		if (this.loaderParams["_feedjson"] != null)
			writeto("zeo['_feedjson'] = " + cincopa.json_stringify(this.loaderParams["_feedjson"]) + ";");
		...
		writeto("go.json_url = '" + this.json_url + "';");
	...
	this.zeOnMediaJSON = function () {
		...
		if (this.MediaJSON.errorcode == "bad_password") {
			...
			return;
		} else if (this.args.allow_lightbox == "thumbnail" || this.args.allow_lightbox == "text") {
			this.showLightbox(this.skin.go.loaderParams._object);
			return;
		}
	...
	this.showLightbox = function(_object){
		var obj = getElement(_object);
		var that = this;
		var items = this.MediaJSON.items;
		if(items.length){
			function closePopup(parent, child) {
				 try{
					 that.skin.player.$media[0].pause();
				 }catch(ex){}
				 fixedContainer.style.display = "none";
				 fixedOverlay.style.display = "none";
				 closeBtn.style.display = "none";
				 doc.body.className = doc.body.className.replace(new RegExp('(\\s|^)'+"cp_overflow_hidden"+'(\\s|$)'),' ');
			}

			var doc = that.detectIframeDocument();
			obj.style.overflow = "visible";
			obj.innerHTML  = "";
			var thumb = this.MediaJSON.items[0].thumbnail_url;
			var img = new Image();
			img.src = thumb;
			img.onload = function(){
				if(this.width < this.height){
					button.className = button.className + " portrait";
				}
			}
			var fixedContainer = document.createElement("div");
			fixedContainer.className = "cp_lightbox_wrap";
			var newObj = document.createElement("div");
			newObj.className = "cp_lightbox_wrap_obj";
			newObj.id = "cp_lightbox_wrap_obj_" + _object;
			fixedContainer.appendChild(newObj);
			var closeBtn = document.createElement("a");
			closeBtn.className = "cp_lightbox_close_btn";
			closeBtn.onclick = function(){
				closePopup();
			}
			var fixedOverlay = document.createElement("div");
			fixedOverlay.className = "cp_lightbox_overlay";
			fixedOverlay.onclick = function(){
				closePopup();
			}
			var button = document.createElement("div");

			if(that.args.allow_lightbox == "thumbnail"){
				button.className = "cp_lightbox_btn";
				var button_inner = document.createElement("div");
				var button_img = document.createElement("img");
				button_img.src = thumb;
				button.appendChild(button_inner);
				button_inner.appendChild(button_img);
				if(that.args.allow_lightbox_size == "small" || that.args.allow_lightbox_size == "medium")
					button.className += " cp_lightbox_btn_"+that.args.allow_lightbox_size;
			} else if(that.args.allow_lightbox == "text"){
				button.className = "cp_lightbox_btn_btn";
				button.innerHTML = "<p>" + that.args.allow_lightbox_text + "</p>"
			}
			...
			button.onclick = function(){
				...
				that.args.allow_lightbox = false;
				that.loaderParams._object = "cp_lightbox_wrap_obj_" + _object;
				that.getMediaJSON();
				doc.body.className += " cp_overflow_hidden";
				try{
			...
			doc.addEventListener("keyup", function(e){
				if(fixedOverlay.style.display != "none" && e.which == 27){
					closePopup();
				}
			})

			obj.appendChild(fixedContainer);
			obj.appendChild(fixedOverlay);
	...
	this.buildLowerPanel = function () {
		...
		var plan_name = this.args.plan_name;
		if (this.acc && this.acc.user)
			plan_name = this.acc.user.plan_name;
	...
	this.setGalleryHTML = function (htm) {
		...
		if (!this.args.allow_lightbox || this.args.allow_lightbox == "no") {
	...
	*/
		...
		try {
			...
			for (var i = 0; i < wind.cincopa.registeredFunctions.length; i++) {
				if (wind.cincopa.registeredFunctions[i].filter == '*') {
					...
					} catch (ex) { cincopa.trace(ex); cincopa.logError("exception-" + name, ex.toString()); }
	...
	this.reportProblem = function (div_id) {
		...
		*/
		...
		var eventName = cincopa.hasTouch() ? "touchstart" : "click";
		reportdivCncBtn.addEventListener(eventName, function (e) {
			e.preventDefault();
			e.stopPropagation();
			reportDiv.outerHTML = "";
			delete reportDiv;
		});
		reportdivSendBtn.addEventListener(eventName, function (e) {
			e.preventDefault();
			e.stopPropagation();
			reportdivErrorMsg.style.display = "none";
...
function Ajax(url, params, callback) {
	...
	function post() {
		...
		http.onreadystatechange = function () {
			cincopa.trace("Ajax onreadystatechsange " + http.readyState + ' ' + http.status);
	...
	try {
		...
		if (cincopa.measure_send.d.length > 0) {
			if (!cincopa.measure_benchmark.sent) {
				...
				cincopa.trace("sending ", cincopa.json_stringify(cincopa.measure_send));
				var oat = new Image(1, 1);
				oat.src = cincopa._ROOT_ANALYTICS + "/os.aspx?a=" + cincopa.json_stringify(cincopa.measure_send);
	...
	try {
		...
		for (var a in cpper) {
			...
			if (theobj != null) {
				...
				cincopa.trace("Performance " + cincopa.json_stringify(m));
				...
				if (m.z > 30) {
					...
					if (cnt >= 10) {
						if (uploadanalytics) {
							var oat = new Image(1, 1);
							oat.src = cincopa._ROOT_ANALYTICS + "/os.aspx?s=" + cincopa.json_stringify(cpmeasuredata);
						}
						cnt = 0;
						cpmeasuredata = [];
					}
...
cincopa.registerEvent(function (eventName, data, gallery) {
	...
	if (cincopa.disable_analytics)
		cincopa.trace("NOT SENDING " + cincopa.json_stringify(ssend));
	else {
		var setref = "";
		if (cincopa.location_blocked)
			setref = "&setref=" + encodeURIComponent(cincopa.location);

		cincopa.trace(cincopa.json_stringify(ssend));
		var oat = new Image(1, 1);
		oat.src = cincopa._ROOT_ANALYTICS + "/ohm.aspx?j=" + encodeURIComponent(cincopa.json_stringify(ssend)) + setref;
	}
...
cincopa.cp_evt_gaevent = function (name, data, gallery) {
	if (name == "runtime.internal-on-args") {
		...
		if (true)
		{
			...
			for (var i = 0; i < gallery.MediaJSON.items.length; i++) {
				if (gallery.MediaJSON.items[i].content_type == "image/jpeg" && gallery.MediaJSON.items[i]["versions"]["jpg_1200x900"]) {
					if (true) {
						...
						jsonld["thumbnailUrl"] = gallery.MediaJSON.items[i]["versions"]["jpg_1200x900"].url;
						...
						var code = cincopa.json_stringify(jsonld);
						var script = document.createElement('script');
				...
				else if (gallery.MediaJSON.items[i].content_type == "video/mp4" && gallery.MediaJSON.items[i]["versions"]["jpg_1200x900"]) {
							...
							if (gallery.MediaJSON.items[i].extra_files[sub].type.startsWith("subtitle-")) {
								...
								client.onreadystatechange = function () {
									...
									if (this.readyState === 4) {
										...
										jsonld = cincopa.json_parse(jsonldx.innerHTML);
										if (jsonld.transcript)
											jsonld.transcript += "\n\n " + txt;
										else
											jsonld.transcript = txt;

										jsonldx.innerHTML = cincopa.json_stringify(jsonld);
									}
								}
								client.send();
							}
	...
	else if ((name == "video.play" || name == "audio.play") && data.item && data.second == 0)
		gallery.pushEventToGA(name + " " + data.item.title + " (" + data.item.rid + ")");
	else if ((name == "video.timeupdate" || name == "audio.timeupdate") && data.item && data.item.duration) {
...
cincopa.cp_send_stats = function () {
	...
	for (uid in cincopa.cp_media_hm) {
		...
		for (rid in cincopa.cp_media_hm[uid]) {
			...
			var lastwritesec = -2;
			var lastvol = 0;
			cincopa.cp_media_hm[uid][rid].totaluniqueseconds = 0;
			var hmlist = "";
			for (sec in cincopa.cp_media_hm[uid][rid]["hm"]) {
				...
				cincopa.cp_media_hm[uid][rid].totaluniqueseconds++;
			...
			if (cincopa.cp_media_hm[uid][rid].totaluniqueseconds > 0) {
				if (lastwritesec != lastsec)
					hmlist += "-" + lastsec; // close range
				if (lastvol > 1)
					hmlist += ":" + lastvol;
				...
				ssend.hm = hmlist;
				ssend.prg = cincopa.cp_media_hm[uid][rid].totaluniqueseconds;
			}

			if (!cincopa.cp_media_hm[uid][rid]["firstsend"] && cincopa.cp_media_hm[uid][rid].totaluniqueseconds > 0) {
				ssend.name = cincopa.cp_media_hm[uid][rid]["cap"];
				ssend.dur = cincopa.cp_media_hm[uid][rid]["dur"];
				if (cincopa.qs && cincopa.qs.cpud)
				{ try { ssend.ud = cincopa.json_parse(cincopa.qs.cpud); } catch (ex) { } }
				else if (cincopa.qs && cincopa.qs.cpeud)
				{ try { ssend.ud = cincopa.json_parse(atob(cincopa.qs.cpeud)); } catch (ex) { } }
...
cincopa.registerEvent(function (eventName, data, gallery) {
	if (data.item.timeline) {
		gallery.onSkinEvent("chapter.show", data);
...
cincopa.registerEvent(function (eventName, data, gallery) {
	...
	if (gallery.skin.player) {
		...
		gallery.onSkinEvent("chapter.hide", data);
	}
}, 'video.unload');
...
cincopa.registerEvent(function (eventName, data, gallery) {
	...
	if (data.skipAction == "linkclick" || time == "preroll") {
		if (data.item.timeline) {
			...
			data.item.timeline = cincopa.json_stringify(timeLineObj);
		}
	}
        ...
        if (timeRial && timeRialwidth) {
            ...
            if (typeof timeLineObj.chapter != "undefined" && (typeof gallery.args.tl_storyboard == 'undefined' || gallery.args.tl_storyboard == "off") && gallery.args.chapter) { // tl_storyboard is an arg which create timeline dots on old timeline skins
                ...
                for (var ind = 0; ind < chaptersArray.length; ind++) {
                    ...
                    chapterSection.style.left = sectionLeft + "px";
...
cincopa.registerEvent(function (eventName, data, gallery) {
    gallery.onSkinEvent("chapter.hide", {});
    gallery.onSkinEvent("chapter.show", data);

    // call function from skin, which will rebuild chapters list
    if(gallery.skin && typeof gallery.skin.buildChapters == "function"){
        gallery.skin.buildChapters(data.item, gallery.skin.player)
    }
}, 'chapter.update');

cincopa.registerEvent(function (eventName, data, gallery) {
    var doc = gallery.detectIframeDocument();
    var container = doc.getElementById(data.containerId || gallery.loaderParams._object);
    ...
    if (player) {
        ...
        if (shareOverlay.length > 0) {
            ...
            var downloadBtn = shareOverlay.getElementsByClassName("download-button");
            ...
            doc.removeEventListener("keyup", gallery.shareHandleKeyUp, true);
            doc.addEventListener("keyup", gallery.shareHandleKeyUp, true);

            function selectText(e) {

                if ("undefined" != typeof window.getSelection && "undefined" != typeof doc.createRange) {
                    var win = doc.defaultView || window;
                    var t = doc.createRange();
                    t.selectNodeContents(e);
                    var i = win.getSelection();
                    i.removeAllRanges(), i.addRange(t)
                } else if ("undefined" != typeof doc.selection && "undefined" != typeof doc.body.createTextRange) {
                    var o = doc.body.createTextRange();
...
cincopa.registerEvent(function (eventName, data, gallery) {
    var container = gallery.detectIframeDocument().getElementById(data.containerId || gallery.loaderParams._object);
    var player = gallery.skin.player;
    if (player) {
        var layers = player.layers[0];
        var controls = player.controls ? player.controls[0] : null;
...
cincopa.contextMenu = function (elemID, options, gallery) {
	var doc = gallery.detectIframeDocument();
	gallery.contextMenuTarget = null;
	...
	if (doc.getElementById("context" + elemID)) {
		...
		for (var i = 0; i < options.length; i++) {
			...
			if (cincopa.hasTouch()) {
				item.ontouchstart = options[i].callback;
			} else {
		...
		if (!cincopa.hasTouch()) {
			elem.addEventListener("contextmenu", function (e) {
				...
				setTimeout(function () {
					...
					gallery.contextMenuTarget = e.target;
				}, 70);
			});
		} else {
			elem.addEventListener("contextmenu", function (e) {
			...
			elem.addEventListener('touchstart', function (e) {
				...
				touchTimer = setTimeout(function () {
					if (touchStartFlag && !isContextMenuEventExist) {
						doc.getElementById('context' + elemID).style.display = 'block';
						doc.getElementById('context' + elemID).style.left = e.changedTouches[0].pageX + "px";
						doc.getElementById('context' + elemID).style.top = e.changedTouches[0].pageY + "px";
						gallery.contextMenuTarget = e.target;
						menuShowed = true;
					}
				}, 1000);
	...
	doc.body.addEventListener("focusout", function (e) {
		if (doc.getElementById('context' + elemID)) {
			setTimeout(function () {
				doc.getElementById('context' + elemID).style.display = 'none';
			}, 70);
...
cincopa.registerEvent(function (eventName, data, gallery) {
	...
	rigth_click_text = "Powered by Cincopa",
    rigth_click_url = "//www.cincopa.com",
    rigth_click_download = false;
	if (gallery.args.allow_change_about_cincopa_text) rigth_click_text = gallery.args.allow_change_about_cincopa_text;
	if (gallery.args.allow_change_about_cincopa_url) rigth_click_url = gallery.args.allow_change_about_cincopa_url;
	...
	if (rigth_click_about) {
		options.push({
			title: rigth_click_text ? rigth_click_text : "Powered by Cincopa",
			callback: function () {
		...
		options.push({
			...
			callback: function () {
				...
				if (typeof data.rcd_cb == "function") {
					src = data.rcd_cb(gallery.contextMenuTarget);
				}
...
cincopa.registerEvent(function (eventName, data, gallery) {
    ...
    var container = gallery.detectIframeDocument().getElementById(gallery.loaderParams._object);

	// check if is bgvideo skin, as this skin creates video element from outside gallery
	var isBackgroundSkin = gallery.detectIframeDocument().getElementById("big-video-wrap_" + gallery.loaderParams._object) !== null;
	if(isBackgroundSkin){
		container = gallery.detectIframeDocument().getElementById("big-video-wrap_" + gallery.loaderParams._object);
	}
    ...
    if(type == "button"){
        ...
        contentDiv.className = "ze_overlay_button ze_overlay";

        var contentInner = document.createElement("div");
        contentInner.className = "ze_overlay_button_inner";
        contentInner.style.color = "#" + gallery.args.overlay_button_text_color;

        var contentDivTitle = document.createElement("span");
        contentDivTitle.className = "ze_overlay_button_title";
        contentDivTitle.innerHTML = gallery.args.overlay_button_title;
        var contentDivBtn = document.createElement("a");
        contentDivBtn.innerHTML = gallery.args.overlay_button_text;
        contentDivBtn.style.backgroundColor = "#" + gallery.args.overlay_button_color;
        contentDivBtn.className = "ze_overlay_button_button";
        contentDivBtn.onclick = function(){
            if(gallery.args.overlay_link)
                window.open(gallery.args.overlay_link, gallery.args.overlay_target ? "_blank" : "_self")
        };
        contentInner.appendChild(contentDivTitle);
        contentInner.appendChild(contentDivBtn);
        contentDiv.appendChild(contentInner);
    } else if(type == "watermark"){
        contentDiv = document.createElement("div");
        contentDiv.className = "ze_overlay_watermark ze_overlay";
        var contentImg = document.createElement("img");
        contentImg.src = gallery.args.overlay_watermark_url;
        contentDiv.onclick = function(){
            if(gallery.args.overlay_link)
                window.open(gallery.args.overlay_link, gallery.args.overlay_target ? "_blank" : "_self")
        };

        if (gallery.args.overlay_watermark_position == 'bottom_left') {
            contentDiv.style.left = 0;
            contentDiv.style.bottom = 0;
        }else if (gallery.args.overlay_watermark_position == 'bottom_right') {
            contentDiv.style.right = 0;
            contentDiv.style.bottom = 0;
        }else if (gallery.args.overlay_watermark_position == 'top_left') {
            contentDiv.style.left = 0;
            contentDiv.style.top = 0;
        }else if (gallery.args.overlay_watermark_position == 'top_right') {
            contentDiv.style.right = 0;
            contentDiv.style.top = 0;
        }
        ...
        contentDiv = document.createElement("div");
        contentDiv.className = "ze_overlay_html ze_overlay";
        if(gallery.args.overlay_html)
            contentDiv.innerHTML = gallery.args.overlay_html;
    }
...
cincopa.registerEvent(function (eventName, data, gallery) {
	// Overlay hide event
    var div_id = data.div_id;
    var container = gallery.detectIframeDocument().getElementById(div_id);
    if(!container) return;
    var overlay = container.getElementsByClassName("ze_overlay");
...
cincopa.registerEvent(function (eventName, data, gallery) {
	// Bind all the events for overlay on html loaded
    if(typeof gallery.args.overlay_type == "undefined" || gallery.args.overlay_type == "off") return;
    var d = {
        type: gallery.args.overlay_type
    }
    if(gallery.args.overlay_time == "onload"){
        gallery.onSkinEvent('overlay.show', d)
    } else if (gallery.args.overlay_time == "onclick"){
        var container = gallery.detectIframeDocument().getElementById(gallery.loaderParams._object);
        if(container){
            var position = window.getComputedStyle(container).getPropertyValue('position');
            if(position != "fixed" && position != "absolute"){
            ...
            var overlay = document.createElement("div");
            overlay.className = "overlay_click_overlay";
    ...
    } else if (gallery.args.overlay_time == "onhover"){
          var container = gallery.detectIframeDocument().getElementById(gallery.loaderParams._object);
          if(container){
              ...
              container.onmouseleave = function(){
                    gallery.onSkinEvent("overlay.hide", {div_id: gallery.loaderParams._object});
              }
          }
    } else if (gallery.args.overlay_time == "at_sec"){
        var time = parseInt(gallery.args.overlay_sec);
        setTimeout(function(){
            gallery.onSkinEvent("overlay.show", d);
        }, time * 1000)
    }
}, 'runtime.on-html-loaded');


cincopa.registerEvent(function (eventName, data, gallery) {
	// create poster on video load 
    if(gallery.args.overlay_type == "poster_logo" && gallery.args.overlay_poster_logo_url){
        var player = gallery.skin.player
        if (player) {
            var layers = player.layers ? player.layers[0] : null;
            if (!layers) return;
            var posterOverlay = layers.getElementsByClassName("mejs-poster");
            if (posterOverlay.length > 0) {
                posterOverlay = posterOverlay[0];
                var logo = posterOverlay.getElementsByClassName("ze-mejs-poster-logo");
                if(logo.length == 0){
                    logo = document.createElement("img");
                    logo.className = "ze-mejs-poster-logo";
                    logo.alt = "logo";
                    logo.src = gallery.args.overlay_poster_logo_url;
                    posterOverlay.appendChild(logo);
                }
...
cincopa.cp_track_fail_load = function (name, data, gallery) {
	if (name == "runtime.internal-on-args") {
		gallery.__track_fail_load = setTimeout(function () { cincopa.logError("fail_load", "timeout"); cincopa.traceit("gallery failed", ""); }, 10000);
	}
...
cincopa.registerEvent(function (eventName, data, gallery) {
	if (data.uid == "AcFA_CB76nb5" || data.uid == "AQDAtmQV2i12") {
		gallery.args.allow_change_about_cincopa_url = "//www.cincopa.com/he/video-hosting";
		gallery.args.allow_change_about_cincopa_text = "סינקופה - וידאו לעסקים";
	}

	if ((gallery.acc && gallery.acc.user && gallery.acc.user.plan_name == "free" && gallery.args.template == "AoIAMJd_cbRb") || cincopa.hash.cpaddlogo) {
...
cincopa.concurrency_counter = cincopa.concurrency_counter || { video:0, lastplay:null };
cincopa.registerEvent("cincopa.cp_handle_concurrency", "runtime.*");
cincopa.registerEvent("cincopa.cp_handle_concurrency", "video.*");
cincopa.registerEvent("cincopa.cp_handle_concurrency", "audio.*");
cincopa.cp_handle_concurrency = function (name, data, gallery) {
	if (name == "runtime.internal-on-args" && gallery.args.skin_code == "/mediaelement/1" && ++cincopa.concurrency_counter.video > 2) {
		gallery.args.fast_start = false;
	}
	else if (name == "video.play") {
		if (cincopa.concurrency_counter.lastplay && cincopa.concurrency_counter.lastplay != gallery) {
			// need to stop the last player 
			//cincopa.concurrency_counter.lastplay.stop(); ?
		}
		cincopa.concurrency_counter.lastplay = gallery;
	}
}

cincopa.hubspot_integration = cincopa.hubspot_integration || {};
cincopa.cp_hubspot_integration = function (name, data, gallery) {
	...
	if (name == "video.load") {
		...
		sendData = {
			http_method: "PUT_JSON", // this is for Ajax
			id: cincopa.cp_media_hm[data.uid][data.item.rid]["hmid"],
			eventTypeId: 19189,
			utk: hubspotutk,
			percentWatched: -1,
			mediaId: data.item.id,
			mediaRid: data.item.rid,
			mediaTitle: data.item.title,
			mediaUrl: data.item.content_url,
			timelineIFrame: {
				"linkLabel": 'View video',
				"iframeLabel": data.item.title,
				"iframeUri": data.item.content_url,
				"width": 800,
				"height": 300
			}
		};		
		...
		var duration = cincopa.cp_hmsToSecondsOnly(data.item.duration);
		var percent = Math.round(cincopa.cp_media_hm[data.uid][data.item.rid].totaluniqueseconds / duration * 100);
		cincopa.hubspot_integration[data.item.rid] = cincopa.hubspot_integration[data.item.rid] || { lastSentPercent: -1 };
		if(percent <= cincopa.hubspot_integration[data.item.rid].lastSentPercent)
			return;
               
		cincopa.hubspot_integration[data.item.rid].lastSentPercent = percent;
		...
		sendData = {
			...
			timelineIFrame: {
				"linkLabel": 'View video',
				"iframeLabel": data.item.title,
				"iframeUri": "https://api.cincopa.com/v2/analytics.single_heatmap_iframe?api_token=1163760iou04m0qzuvzikzpb3sverp54qwe&date="+(new Date()).toISOString().slice(0,-14)+"&zone=0&hm="+cincopa.cp_media_hm[data.uid][data.item.rid]["hmid"]+"&disable_editor=y", //data.item.content_url,
				"width": 600,
				"height": 650
			}
		};		
		...
		sendData = {
			...
			mediaTitle: data.item.title,
			email: data.formData.email,
			firstName: data.formData.first,
			lastName: data.formData.last
		};		
	}

	if (sendData) {
		// send to Hubspot
		var apiEndpoint = 'https://fiaz4fns1b.execute-api.eu-central-1.amazonaws.com/latest/hubspot/api';
		var appId = 43261;
		Ajax(apiEndpoint + '/integrations/v1/' + appId + '/timeline/event' + '?uid=' + data.uid, sendData, function (res) {
			cincopa.trace("hubspot-event", res);
		});
	}
}
